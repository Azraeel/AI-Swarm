PlatoonTemplate { Name = 'T1AirDefaultQueue',
    FactionSquads = {
        UEF = {
            { 'uea0101', 1, 2, 'Attack', 'none' },		-- Scout
            { 'uea0102', 1, 5, 'Attack', 'none' },		-- Interceptor
			{ 'uea0103', 1, 2, 'Attack', 'none' },	    -- Bomber
         },
        Aeon = {
        	{ 'uaa0101', 1, 2, 'Attack', 'none' },		-- Scout
            { 'uaa0102', 1, 5, 'Attack', 'none' },		-- Interceptor
			{ 'uaa0103', 1, 2, 'Attack', 'none' },	    -- Bomber
        },
        Cybran = {
        	{ 'ura0101', 1, 2, 'Attack', 'none' },		-- Scout
            { 'ura0102', 1, 6, 'Attack', 'none' },		-- Interceptor
			{ 'ura0103', 1, 2, 'Attack', 'none' },	    -- Bomber
            { 'xra0105', 1, 1, 'Attack', 'none' },		-- Gunship
        },
        Seraphim = {
        	{ 'xsa0101', 1, 2, 'Attack', 'none' },		-- Scout
            { 'xsa0102', 1, 5, 'Attack', 'none' },		-- Interceptor
			{ 'xsa0103', 1, 2, 'Attack', 'none' },	    -- Bomber
        },
    }
}

PlatoonTemplate { Name = 'T2AirDefaultQueue',
    FactionSquads = {
        UEF = {
        	{ 'uea0101', 1, 1, 'Attack', 'none' },      -- Scout
        	{ 'uea0102', 1, 6, 'Attack', 'none' },		-- Interceptor
            { 'dea0202', 1, 3, 'Attack', 'none' },      -- Fighter/Bomber
            { 'uea0203', 1, 1, 'Attack', 'none' },      -- Gunship
         },
        Aeon = {
        	{ 'uaa0101', 1, 1, 'Attack', 'none' },		-- Scout
            { 'xaa0202', 1, 8, 'Attack', 'none' },      -- Fighter
            { 'uaa0203', 1, 3, 'Attack', 'none' },      -- Gunship
        },
        Cybran = {
        	{ 'ura0101', 1, 1, 'Attack', 'none' },		-- Scout
        	{ 'ura0102', 1, 6, 'Attack', 'none' },		-- Interceptor
            { 'dra0202', 1, 3, 'Attack', 'none' },      -- Fighter/Bomber
            { 'ura0203', 1, 2, 'Attack', 'none' },      -- Gunship
        },
        Seraphim = {
        	{ 'xsa0101', 1, 1, 'Attack', 'none' },		-- Scout
        	{ 'xsa0102', 1, 6, 'Attack', 'none' },		-- Interceptor
            { 'xsa0202', 1, 3, 'Attack', 'none' },      -- Fighter/Bomber
            { 'xsa0203', 1, 2, 'Attack', 'none' },      -- Gunship
        },
    }
}

PlatoonTemplate { Name = 'T3AirDefaultQueue',
    FactionSquads = {
        UEF = {
        	{ 'uea0302', 1, 2, 'Attack', 'none' },      -- Scout
            { 'uea0303', 1, 7, 'Attack', 'none' },      -- ASF
            { 'uea0304', 1, 2, 'Attack', 'none' },      -- Bomber
            { 'uea0305', 1, 2, 'Attack', 'none' },      -- Gunship
         },
        Aeon = {
        	{ 'uaa0302', 1, 2, 'Attack', 'none' },      -- Scout
            { 'uaa0303', 1, 7, 'Attack', 'none' },      -- ASF
            { 'uaa0303', 1, 2, 'Attack', 'none' },      -- Bomber
            { 'xaa0305', 1, 2, 'Attack', 'none' },      -- Gunship
        },
        Cybran = {
            { 'ura0302', 1, 2, 'Attack', 'none' },      -- Scout
            { 'ura0303', 1, 7, 'Attack', 'none' },      -- ASF
            { 'ura0304', 1, 2, 'Attack', 'none' },      -- Bomber
            { 'xra0305', 1, 2, 'Attack', 'none' },      -- Gunship
        },
        Seraphim = {
        	{ 'xsa0302', 1, 2, 'Attack', 'none' },      -- Scout
            { 'xsa0303', 1, 8, 'Attack', 'none' },      -- Interceptor
            { 'xsa0304', 1, 2, 'Attack', 'none' },      -- Bomber
        },
    }
}
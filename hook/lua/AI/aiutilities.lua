...
function AIFindNearestCategoryTargetInRangeSwarm(aiBrain, platoon, squad, position, maxRange, MoveToCategories, TargetSearchCategory, enemyBrain)
    if not maxRange then
        --LOG('* AI-Swarm: AIFindNearestCategoryTargetInRangeSwarm: function called with empty "maxRange"' )
        return false, false, false, 'NoRange'
    end
    if not TargetSearchCategory then
        --LOG('* AI-Swarm: AIFindNearestCategoryTargetInRangeSwarm: function called with empty "TargetSearchCategory"' )
        return false, false, false, 'NoCat'
    end
    if not position then
        --LOG('* AI-Swarm: AIFindNearestCategoryTargetInRangeSwarm: function called with empty "position"' )
        return false, false, false, 'NoPos'
    end
    if not platoon then
        LOG('* AI-Swarm: AIFindNearestCategoryTargetInRangeSwarm: function called with no "platoon"' )
        return false, false, false, 'NoPos'
    end
    ...
    for _, range in RangeList do
        ...
        for _, v in MoveToCategories do
            ...
            for num, Target in TargetsInRange do
                ...
                if not ValidateLayerSwarm(TargetPosition, platoon.MovementLayer) then continue end
                ...
                --LOG('* AIFindNearestCategoryTargetInRangeSwarm: canAttack '..repr(canAttack))
                if not Target.Dead and EntityCategoryContains(category, Target) and canAttack then
                    ...
                    --LOG('* AIFindNearestCategoryTargetInRangeSwarm: targetRange '..repr(targetRange))
                    if targetRange < distance then
                        ...
                        if path then
                            ...
                            --LOG('* AIFindNearestCategoryTargetInRangeSwarm: Possible target with path. distance '..distance..'  ')
                        -- We don't find a path with markers
                            ...
                            elseif reason == 'NoGraph' then
                                ...
                                if success then
                                    UnitWithPath = Target
                                    distance = targetRange
                                    --LOG('* AIFindNearestCategoryTargetInRangeSwarm: Possible target with CanPathTo(). distance '..distance..'  ')
                                ...
                                -- There is no path to the target.
                                    ...
                                    UnitNoPath = Target
                                    distance = targetRange
                                    --LOG('* AIFindNearestCategoryTargetInRangeSwarm: Possible target failed CanPathTo(). distance '..distance..'  ')
                                end
                            end
                        end
                    end
...
function AIFindNearestCategoryTargetInRangeSwarmCDRSwarm(aiBrain, position, maxRange, MoveToCategories, TargetSearchCategory, enemyBrain)
    if type(TargetSearchCategory) == 'string' then
        TargetSearchCategory = ParseEntityCategory(TargetSearchCategory)
    end
    local enemyIndex = false
...
function AIFindNearestCategoryTeleportLocationSwarm(aiBrain, position, maxRange, MoveToCategories, TargetSearchCategory, enemyBrain)
    if type(TargetSearchCategory) == 'string' then
        TargetSearchCategory = ParseEntityCategory(TargetSearchCategory)
    end
    local enemyIndex = false
    ...
    --LOG('* AIFindNearestCategoryTeleportLocationSwarm: numTargets '..table.getn(TargetsInRange)..'  ')
    --DrawCircle(position, range, '0000FF')
    for _, v in MoveToCategories do
        category = v
        if type(category) == 'string' then
            ...
            if not Target.Dead and EntityCategoryContains(category, Target) then
                ...
                if targetRange < distance then
                    ...
                    if categories.ANTITELEPORT then 
                        ...
                        --LOG('* AIFindNearestCategoryTeleportLocationSwarm: numAntiteleportUnits '..table.getn(AntiteleportUnits)..'  ')
                        ...
                        for i, unit in AntiteleportUnits do
                            ...
                            if NoTeleDistance then
                                ...
                                if dist and NoTeleDistance >= dist then
                                    --LOG('* AIFindNearestCategoryTeleportLocationSwarm: Teleport Destination Scrambled 1 '..repr(TargetPosition)..' - '..repr(AntiTeleportTowerPosition))
                                    scrambled = true
                                    break
                                end
                            end
                    ...
                    --LOG('* AIFindNearestCategoryTeleportLocationSwarm: Found a target that is not Teleport Scrambled')
                    TargetUnit = Target
                    distance = targetRange
                end
            end

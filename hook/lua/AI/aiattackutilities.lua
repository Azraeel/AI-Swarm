WARN('['..string.gsub(debug.getinfo(1).source, ".*\\(.*.lua)", "%1")..', line:'..debug.getinfo(1).currentline..'] * AI-Swarm: offset aiattackutilities.lua' )
...
--[[function GetBestThreatTarget(aiBrain, platoon, bSkipPathability)
    ...
    -- get threat at position
    local threatTable = aiBrain:GetThreatsAroundPosition(position, 16, true, 'Overall')

    -- for every threat...
    for _,threat in threatTable do

        -- get position of the threat
        local ThreatPos = {threat[1], 0, threat[2]}

        -- check if we can path to here safely... give a large threat weight to sort by threat first
        local path, reason = PlatoonGenerateSafePathTo(aiBrain, platoon.MovementLayer, position, ThreatPos , 512)

        -- clear command queue
        platoon:Stop()

        -- use if possible at 250
        if VDist2Sq(position[1], position[3], ThreatPos[1], ThreatPos[3]) > 256*256 then
            SendPlatoonWithTransportsNoCheck(aiBrain, platoon, ThreatPos, false)
        end

        -- if we can path to here safely...
        if path then

            -- store best threat position
            local bestDistSq = 0
            local bestThreat = 0

            -- for every node of the path... check threat and save the best location with least threat.
            for _,p in path do

                -- get threat at position on map
                local distSq = VDist2Sq(position[1], position[3], p[1], p[3])

                local threat = aiBrain:GetThreatAtPosition({p[1],0,p[3]}, 1, true) * 4 + aiBrain:GetThreatAtPosition({p[1],0,p[3]}, 0, true)

                -- save position if threat is least
                if distSq > 512*512 then
                    -- use path size to determine value of path
                    local pathSize = table.getn(path)
                    local threatWeight = (threat * (1 / math.max(pathSize, 1))) + (distSq / (math.max(pathSize, 1) * math.max(pathSize, 1)))

                    if not bSkipPathability and AIAttackUtils.GetSurfaceThreatOfUnits(platoon) < 1 then
                        continue
                    end

                    if threatWeight < bestDistSq or bestDistSq == 0 then
                        bestDistSq = threatWeight
                        bestThreat = threat - distSq / 512 + aiBrain:GetThreatAtPosition({p[1],0,p[3]}, 0, true)
                        bestPos = p
                    end
                elseif threat < bestThreat or bestThreat == 0 then
                    -- use path size to determine value of path
                    local pathSize = table.getn(path)
            ...
            -- return position of best threat
            if bestThreat > 0 then
                return bestPos, reason
            elseif not bSkipPathability and AIAttackUtils.GetSurfaceThreatOfUnits(platoon) < 1 then
                continue
            elseif threatTable[1][3] > 0 then
                local tempDist = VDist2Sq(position[1], position[3], threatTable[1][1], threatTable[1][2])

                if tempDist < (512*512) and (threatTable[1][4] - GetThreatAtPosition(aiBrain, {threatTable[1][1],0,threatTable[1][2]}, 1, true)) > 0 then

                    local pathSize = table.getn(path) + 1 -- for the extra move at beginning

                    local threatWeight = (threatTable[1][4] * (1 / math.max(pathSize, 1))) + (tempDist / (math.max(pathSize, 1) * math.max(pathSize, 1)))

                    if threatWeight < bestDistSq or bestDistSq == 0 then
                        bestDistSq = threatWeight
                        bestThreat = threatTable[1][4] - tempDist / 512 + aiBrain:GetThreatAtPosition({threatTable[1][1],0,threatTable[1][2]}, 0, true)
                        bestPos = {threatTable[1][1],0,threatTable[1][2]}
                    end
